...
def process_video(filename, x_start=0, y_start=0, angle_start=0, crop=1):
    ...
    frame_previous = frame_previous[:int((frame_previous.shape[0] * crop) // 1), :]
    ...
    while True:
        ...
        frame = frame[:int((frame.shape[0]*crop) // 1), :] 
...
if __name__ == '__main__':
    ...
    print(process_video('drive_0'))
    print(process_video('drive_0', crop=0.8))
    print(process_video('drive_0', crop=0.5))
    print(process_video('drive_0', crop=0.3))
    
    '''
    (36.7116058692371, 204.22962307464235, 0.4388365099517174)
    (19.584603243859448, 100.27333137445805, 0.4227328314457064)
    (-1.715769835273885, -2.206988055799586, 0.09911910495023506)
    (-5.579785390183896, -27.019096538115914, 0.2162011708015151)
    '''
...
#     frame_previous = frame_previous[:frame_previous.shape[0] // 2, :] 
